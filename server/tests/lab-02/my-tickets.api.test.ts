...
const prisma = vi.hoisted(() => ({ user: { findUnique: vi.fn() }, developmentRequester: { findFirst: vi.fn() }, ticket: { findMany: vi.fn(), count: vi.fn() } }));
let authHeader = "";
...
describe("GET /api/tickets", () => {
  ...
  it("returns only the selected requester tickets with default pagination", async () => {
    ...
    const response = await request(app).get("/api/tickets?requesterId=1").set("Authorization", authHeader);
  ...
  it("applies search, every filter, sorting, and pagination", async () => {
    ...
    const response = await request(app).get("/api/tickets?requesterId=1&search=vpn&categoryId=2&relatedSystemId=3&requestedPriority=HIGH&status=NEW&sort=summary&direction=asc&page=2&pageSize=20").set("Authorization", authHeader);
    expect(response.status).toBe(200);
  ...
  it("requires requester context before querying tickets", async () => {
    const response = await request(app).get("/api/tickets").set("Authorization", authHeader);
  ...
  it("rejects oversized integer query values safely", async () => {
    const response = await request(app).get("/api/tickets?requesterId=999999999999999999999999").set("Authorization", authHeader);
    expect(response.status).toBe(400);
  ...
  it("rejects malformed query values safely", async () => {
    const response = await request(app).get("/api/tickets?requesterId=bad&pageSize=7").set("Authorization", authHeader);
    expect(response.status).toBe(400);
    expect(response.body).toEqual({ error: "requesterId must be a positive integer." });
    expect(prisma.developmentRequester.findFirst).not.toHaveBeenCalled();
  });
