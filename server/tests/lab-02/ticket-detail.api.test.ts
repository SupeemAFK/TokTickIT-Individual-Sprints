...
const prisma = vi.hoisted(() => ({ user: { findUnique: vi.fn() }, developmentRequester: { findFirst: vi.fn() }, ticket: { findFirst: vi.fn() } }));
let authHeader = "";
vi.mock("../../src/prisma.js", () => ({ getPrisma: () => prisma }));
import { app } from "../../src/app.js";

describe("GET /api/tickets/:ticketId", () => {
  beforeEach(async () => { vi.resetAllMocks(); configureRequesterAuth(prisma.user.findUnique); authHeader = "Bearer " + await loginRequester(app); prisma.developmentRequester.findFirst.mockResolvedValue({ id: 1 }); });
  it("returns an owned ticket with read-only display data", async () => {
    ...
    const response = await request(app).get("/api/tickets/8?requesterId=1").set("Authorization", authHeader);
  ...
  it("rejects invalid ticket IDs safely", async () => {
    const response = await request(app).get("/api/tickets/nope?requesterId=1").set("Authorization", authHeader);
  ...
  it("does not disclose missing or non-owned tickets", async () => {
    ...
    const response = await request(app).get("/api/tickets/8?requesterId=2").set("Authorization", authHeader);
    expect(response.status).toBe(404);
    expect(response.body).toEqual({ error: "Ticket not found." });
  });
})
