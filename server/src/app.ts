...
app.get("/api/tickets/:ticketId", async (req, res) => { try { const ticket = await ownedTicket(ticketId(req), req); if (!ticket) return fail(res, 404, "Ticket not found.", "NOT_FOUND"); res.json(ticketDetail(ticket, false)); } catch (error) { handle(res, error, "Unable to load ticket."); } });
...
app.post("/api/tickets", async (req, res) => {
  try {
    ...
    const ticket = await (getPrisma() as any).ticket.findUnique({ where: { id }, include: detailInclude }); res.status(201).json(ticketDetail(ticket, false));
...
app.post("/api/tickets/:ticketId/resolution-signal", async (req, res) => { try { const ticket = await ownedTicket(ticketId(req), req, {}); if (!ticket) return fail(res, 404, "Ticket not found.", "NOT_FOUND"); const updated = await (getPrisma() as any).ticket.update({ where: { id: ticket.id }, data: { requesterReportedResolvedAt: new Date() }, include: detailInclude }); res.json(ticketDetail(updated, false)); } catch (error) { handle(res, error, "Unable to record the resolution signal."); } });
