  ...
  return (
    <div className="min-vh-100 toktickit-page">
      ...
      <main className="container py-5 toktickit-page" style={{ maxWidth: 960 }}>
        ...
        {activePage === "create" ? <CreateTicketForm requester={selectedRequester} onCancel={() => setActivePage("tickets")} /> : activePage === "detail" && selectedTicket ? <TicketDetail ticketId={selectedTicket} requester={selectedRequester} onBack={() => setActivePage("tickets")} /> : <MyTickets requester={selectedRequester} onCreateTicket={() => setActivePage("create")} onOpenTicket={(ticket) => { setSelectedTicket(ticket.id); setActivePage("detail"); }} />}
      </main>
    </div>
  );
}
