...
export default function MyTickets({ requester, onCreateTicket, onOpenTicket }: { requester: DevelopmentRequester; onCreateTicket: () => void; onOpenTicket?: (id: number) => void }) {
...
function Results({ items, onOpenTicket }: { items: TicketListItem[]; onOpenTicket?: (id: number) => void }) { return <div className="mt-4"><table className="table align-middle d-none d-md-table"><thead><tr><th>Number</th><th>Summary</th><th>Category</th><th>Priority</th><th>Status</th><th>Updated</th></tr></thead><tbody>{items.map((ticket) => <tr key={ticket.id} className={onOpenTicket ? "ticket-row" : undefined} onClick={() => onOpenTicket?.(ticket.id)}><td><strong>{ticket.ticketNumber}</strong></td><td>{ticket.summary}</td><td>{ticket.category.name}</td><td><span className={`badge text-bg-${ticket.requestedPriority === "HIGH" ? "danger" : ticket.requestedPriority === "MEDIUM" ? "warning" : "success"}`}>{priority(ticket.requestedPriority)}</span></td><td><span className="badge text-bg-secondary">New</span></td><td>{date(ticket.updatedAt)}</td></tr>)}</tbody></table><div className="d-md-none">{items.map((ticket) => <button key={ticket.id} className="card w-100 text-start mb-3 ticket-card" onClick={() => onOpenTicket?.(ticket.id)}><div className="card-body"><strong>{ticket.ticketNumber}</strong><div className="mt-1">{ticket.summary}</div><div className="small text-secondary mt-2">{ticket.category.name} · Updated {date(ticket.updatedAt)}</div><div className="mt-2"><span className="badge text-bg-secondary me-2">New</span><span className="badge text-bg-success">{priority(ticket.requestedPriority)}</span></div></div></button>)}</div></div>; }
