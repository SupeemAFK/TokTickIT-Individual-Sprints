...
describe("Lab 3 first-login password change", () => {
  ...
  it("renders the password form instead of logging out", async () => { render(<App />); expect(await screen.findByRole("heading", {name:/change your initial password/i})).toBeInTheDocument(); const user=userEvent.setup(); await user.type(screen.getByLabelText(/new password/i), "NewSecure123"); await user.type(screen.getByLabelText(/confirm password/i), "NewSecure123"); expect(screen.getByRole("button", {name:/save password/i})).toBeInTheDocument(); });
});
